...
def clasificar_criterios_gemini(rid: int, nombre: str, serie_reviews) -> dict:
    ...
    if clave_cache in _cache_criterios:
        return _cache_criterios[clave_cache]
...
def analizar_restaurante(df_r):
    ...
    criterios = clasificar_criterios_gemini(rid, nombre, df_r['Review'].tolist())
    ...
    # Personal
    if MODO_ECONOMICO:
        # Reusar personal_destacado del CSV previo si existe, si no dejar vacío
        personal_str = ''
