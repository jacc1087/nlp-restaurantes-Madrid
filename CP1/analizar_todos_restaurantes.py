...
NO_PLATOS = {
    ...
    'restaurante','comida','servicio','trato','atención','experiencia','lugar',
...
# ─────────────────────────────────────────────────────────────────────────────
# Cache para validación Gemini de platos (evitar llamadas repetidas)
_cache_es_plato: dict = {}

def filtrar_platos_con_gemini(candidatos: list) -> list:
    """
    ...
    # Primero separar los que ya están cacheados
    resultado = []
    por_consultar = []
    for nombre, cuenta in candidatos:
        if nombre in PLATOS_WHITELIST or any(
            nombre in wl or nombre.startswith(wl + ' ') or nombre.endswith(' ' + wl)
            for wl in PLATOS_WHITELIST
        ):
            resultado.append((nombre, cuenta))  # whitelist: aprobado directo
    ...
    # Sin Gemini: aplicar solo heurística extra
    if not _GEMINI_KEY:
        for nombre, cuenta in por_consultar:
            tokens = nombre.split()
            # Rechazar tokens simples que parecen adjetivos/adverbios
            if len(tokens) == 1 and PATRONES_NO_PLATO.search(nombre):
                _cache_es_plato[nombre] = False
            else:
                _cache_es_plato[nombre] = True  # sin Gemini, duda -> incluir
                resultado.append((nombre, cuenta))
        return resultado

    # Con Gemini: preguntar en lote
    nombres_consultar = [n for n, _ in por_consultar]
    prompt = (
        f"Estos son candidatos extraídos de reseñas de restaurantes: {nombres_consultar}\n"
        "Clasifica cada uno:\n"
        "- es_plato: nombres de comidas, ingredientes, bebidas, platos, o términos gastronómicos\n"
        "- no_es_plato: adjetivos, adverbios, sustantivos genéricos, verbos, experiencias, personas\n"
        "Ejemplos es_plato: pulpo, tikka masala, paella, croquetas, vino, risotto\n"
        "Ejemplos no_es_plato: acogedor, destacar, principal, rápido, volveré, gran, verdad\n"
        "Si hay duda mínima, pon en no_es_plato.\n"
        'Responde SOLO con JSON: {"es_plato": [...], "no_es_plato": [...]}'
    )
    ...
    for nombre, cuenta in por_consultar:
        if nombre in aprobados:
            ...
            resultado.append((nombre, cuenta))
        elif nombre in rechazados:
            _cache_es_plato[nombre] = False
        else:
            # No clasificado: heurística de último recurso
            _cache_es_plato[nombre] = True
            resultado.append((nombre, cuenta))
...
def extraer_platos(serie_reviews, n=5):
    ...
    # Candidatos con >= 2 menciones
    candidatos = [(t, freq_resenas[t]) for t in freq_resenas if freq_resenas[t] >= 2]
    ...
    # Filtro Gemini (o whitelist si no hay key)
    platos_filtrados = filtrar_platos_con_gemini(platos_sin_filtrar)

    # Ordenar por frecuencia y recortar a n
    platos_filtrados.sort(key=lambda x: -x[1])
    platos = platos_filtrados[:n]

    return normalizar_platos(platos)
...
def analizar_restaurante(df_r):
    ...
    platos_top  = extraer_platos(df_r['Review'], n=5)
...
if os.path.exists(OUTPUT_CSV):
    ...
    ids_procesados = set(df_prev['id_restaurante'].astype(int).tolist())
    filas_resumen  = df_prev.to_dict('records')
    print(f"CSV existente: {len(ids_procesados)} restaurantes ya procesados.")

if os.path.exists(OUTPUT_RESENAS):
    filas_resenas = [pd.read_csv(OUTPUT_RESENAS)]

ids_pendientes = [i for i in ids_todos if int(i) not in ids_procesados]
print(f"Total: {len(ids_todos)} | Procesados: {len(ids_procesados)} | Pendientes: {len(ids_pendientes)}\n")
